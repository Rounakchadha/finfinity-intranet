...
function AssetRequestModal({ isOpen, onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // submission error handled by finally block
    } finally {
...
const ITToolsPage = () => {
    ...
    const fetchDropdownData = async () => {
        try {
            const [typesRes, locationsRes] = await Promise.all([
                fetch('/api/assets/types', { credentials: 'include' }),
    ...
    const fetchAuditLogs = async (assetTag = '') => {
        setLoading(true);
        try {
            const url = assetTag
                ? `/api/assets/audit-logs?asset_tag=${encodeURIComponent(assetTag)}`
                : '/api/assets/audit-logs';
    ...
    const fetchAssetRequests = async () => {
        ...
        try {
            ...
            const data = await res.json();
            setAssetRequests(Array.isArray(data) ? data : []);
        } catch {
            setAssetRequests([]);
        } finally {
            setLoading(false);
    ...
    const handleReviewRequest = async (id, status) => {
        const reviewNotes = window.prompt(`Notes for ${status} (optional):`);
        if (reviewNotes === null) return; // cancelled
        try {
            const res = await fetch(`/api/asset-requests/${id}/review`, {
                method: 'PUT',
                credentials: 'include',
                headers: {
    ...
    const fetchAssets = async (filter = 'all') => {
        setLoading(true);
        try {
            const response = await fetch(`/api/assets?filter=${filter}`, { credentials: 'include' });
            const data = await response.json();
    ...
    const handleAllocateAsset = async (assetTag, userEmail) => {
        try {
            const response = await fetch('/api/assets/allocate', {
                ...
                body: JSON.stringify({ asset_tag: assetTag, user_email: userEmail })
            });
    ...
    const handleReallocateAsset = async (assetTag, newUserEmail) => {
        try {
            const response = await fetch('/api/assets/reallocate', {
                ...
                body: JSON.stringify({ asset_tag: assetTag, new_user_email: newUserEmail })
            });
    ...
    const handleDecommissionAssets = async () => {
        ...
        try {
            const response = await fetch('/api/assets/decommission', {
                ...
                body: JSON.stringify({ asset_tags: selectedAssets })
            });
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    {activeModal.type === 'audit-logs' && (
                        <div className="space-y-4">
                            <p className="text-sm text-gray-500">Showing last 200 events across all assets.</p>
                            <div className="overflow-auto" style={{ maxHeight: '480px' }}>
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {auditLogs.length === 0 ? (
                                            <tr>
                                                <td colSpan={6} className="p-4 text-center text-gray-400">No audit logs found.</td>
                                            </tr>
                                        ) : auditLogs.map((log, i) => (
                                            <tr key={log.id} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                                <td className="p-2 font-mono text-xs">{log.asset_tag}</td>
                                                <td className="p-2">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                                        log.action === 'created'       ? 'bg-blue-100 text-blue-700' :
                                                        log.action === 'allocated'     ? 'bg-green-100 text-green-700' :
                                                        log.action === 'deallocated'   ? 'bg-yellow-100 text-yellow-700' :
                                                        log.action === 'reallocated'   ? 'bg-purple-100 text-purple-700' :
                                                        'bg-red-100 text-red-700'
                                                    }`}>
                                                        {log.action}
                                                    </span>
                                                </td>
                                                <td className="p-2 text-xs">{log.performed_by_name}<br/><span className="text-gray-400">{log.performed_by_email}</span></td>
                                                <td className="p-2 text-xs">{log.from_user_email || '—'}</td>
                                                <td className="p-2 text-xs">{log.to_user_email || '—'}</td>
                    ...
                    {activeModal.type === 'asset-requests' && (
                        <div className="space-y-4">
                            <p className="text-sm text-gray-500">Review employee asset requests. Pending requests require your action.</p>
                            <div className="overflow-auto" style={{ maxHeight: '480px' }}>
                                <table className="w-full text-sm">
                                    <thead className="sticky top-0 bg-gray-100">
                                        <tr>
                                    ...
                                    <tbody>
                                        {assetRequests.length === 0 ? (
                                            <tr>
                                                <td colSpan={6} className="p-4 text-center text-gray-400">No requests found.</td>
                                            </tr>
                                        ) : assetRequests.map((req, i) => (
                                            <tr key={req.id} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                                <td className="p-2 text-xs">{req.employee_name}<br/><span className="text-gray-400">{req.employee_email}</span></td>
                                                <td className="p-2">{req.asset_type}</td>
                                                <td className="p-2 text-xs text-gray-600">{req.notes || '—'}</td>
                                                <td className="p-2">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                                ...
                                                <td className="p-2">
                                                    ...
                                                    {req.status !== 'pending' && (
                                                        <span className="text-xs text-gray-400">{req.review_notes || '—'}</span>
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                ...
                <button
                    onClick={() => {
                        fetchAuditLogs();
                        setActiveModal({ type: 'audit-logs', title: 'Asset Audit Log' });
                    }}
                    className="bg-gray-700 hover:bg-gray-800 text-white p-4 rounded-lg transition-colors"
                >
                    <div className="text-center">
                        <div className="text-2xl mb-2">📋</div>
                ...
                <button
                    onClick={() => {
                        fetchAssetRequests();
                        setActiveModal({ type: 'asset-requests', title: 'Asset Requests' });
                    }}
                    className="bg-teal-600 hover:bg-teal-700 text-white p-4 rounded-lg transition-colors"
                >
                    <div className="text-center">
                        <div className="text-2xl mb-2">📨</div>

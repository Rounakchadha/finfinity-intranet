...
# Ensure APP_KEY is set: prefer the env var injected by Docker (env_file),
# fall back to what's in /var/www/html/.env, or generate a fresh one.
if [ -z "$APP_KEY" ]; then
    APP_KEY_FROM_FILE=$(grep '^APP_KEY=' /var/www/html/.env | cut -d '=' -f2-)
